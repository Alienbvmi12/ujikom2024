<script>
    ...
    $(document).ready(function() {
        ...
        table = $("#main-table").DataTable({
            serverSide: true,
            ajax: {
                url: base_url + "petugas/produk/read/",
                dataSrc: 'data'
            },
            order: [0, "desc"],
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        if (row.status == 1) {
                            ...
                            <button class="btn btn-danger" onclick="aktif(this, 0)">
                            ...
                            <button class="btn btn-success" onclick="aktif(this, 1)">
    ...
    function create() {
        ...
        NProgress.start();
        $.ajax({
            url: base_url + "petugas/produk/create/",
            type: "post",
            data: formData,
            contentType: false,
            processData: false,
    ...
    //     NProgress.start();
    //     $.ajax({
    //         url: base_url + "petugas/produk/update/",
    //         type: "post",
    //         data: formData,
    //         contentType: false,
    //         processData: false,
    ...
    function deleteM(context) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                NProgress.start();
                $.ajax({
                    url: base_url + "petugas/produk/delete/" + id + "/",
                    type: "post",
                    contentType: false,
                    processData: false,
                    success: function(response) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (result.value != "") {
                    ...
                    $.ajax({
                        url: base_url + "petugas/produk/tam_stok/",
                        type: "post",
                        data: form,
                        contentType: false,
                        processData: false,
    ...
    function aktif(context, status) {
        ...
        Swal.fire({
            title: 'Konfirmasi',
            text: 'Apakah anda yakin untuk ' + stat + ' produk ini?',
            icon: 'warning',
            showCancelButton: true,
        }).then((result) => {
            if (result.isConfirmed) {
                if (status == 1 || status == 0) {
                    $.ajax({
                        url: base_url + "petugas/produk/aktif/" + id + "/" + status + "/",
                        type: "post",
                        contentType: false,
                        processData: false,
                        success: function(response) {
